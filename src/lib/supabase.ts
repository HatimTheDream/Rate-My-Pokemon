...
export async function getRating(userId: string, dex: number, shiny: boolean): Promise<number | null> {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) console.error('getRating error:', error);
  return data?.rating ?? null;
}

export async function getRatingsForUser(userId: string): Promise<Record<number, number>> {
  const { data, error } = await supabase
    .from('user_ratings')
...
export async function saveRating(userId: string, dex: number, rating: number, shiny: boolean): Promise<void> {
  const effectiveDex = shiny ? dex * -1 : dex;
  const { error } = await supabase
    .from('user_ratings')
    .upsert(
      { user_id: userId, pokemon_dex: effectiveDex, rating, updated_at: new Date().toISOString() },
      { onConflict: 'user_id,pokemon_dex' }
    );
  if (error) throw error;
}

export async function addRatingEvent(userId: string, dex: number, rating: number): Promise<void> {
  const { error } = await supabase
    .from('rating_events')
...
export async function getRatingEvents(userId: string, dex: number): Promise<{ t: number; y: number }[]> {
  const { data, error } = await supabase
    .from('rating_events')
