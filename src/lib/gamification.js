...
export async function awardCoinsForRating(userId) {
    ...
    // Log transaction
    await addCoinTransaction(userId, totalCoins, 'earn_rating', `Rating reward${streakBonus > 0 ? ` (+${streakBonus} streak bonus)` : ''}`);
...
export async function addCoinTransaction(userId, amount, type, description) {
...
export async function purchaseShopItem(userId, itemId) {
    ...
    // Log transaction
    await addCoinTransaction(userId, -item.price, 'purchase', `Purchased ${item.display_name}`);
    return {
        success: true,
